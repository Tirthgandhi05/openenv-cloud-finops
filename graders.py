# graders.py — deterministic episode scoring
...
def run_grader(env) -> GraderResult:
    ...
    if task_id == "task_1":
        ...
        extra = f" Orphans remaining: {remaining_orphans}/8."
    else:
        extra = ""

    return GraderResult(
        task_id=task_id, score=round(final, 4),
        money_saved=round(money_saved, 2), savings_target=target,
        downtime_events=env._downtime_events, false_kills=env._false_kills,
        honeypot_hits=env._honeypot_hits, sequence_violations=env._sequence_violations,
        breakdown=breakdown,
        message=f"{verdict} Saved ${money_saved:,.0f} of ${target:,.0f} target.{extra}",
    )
